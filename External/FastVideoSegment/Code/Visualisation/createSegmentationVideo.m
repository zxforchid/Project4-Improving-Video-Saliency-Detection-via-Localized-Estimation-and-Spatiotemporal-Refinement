...
function createSegmentationVideo( options, params, data, mode )
    ...
    folder = fullfile( options.outfolder, 'visuals' );
    if( ~exist( folder, 'dir' ) )
        mkdir( folder );
    end
    
    if( isfield( params, 'range' ) && ~isempty( params.range ) )
        range = params.range;
    else
        range = 1;
    end
    ...
    if( isfield( params, 'name' ) && ~isempty( params.name ) )
        ...
        filename = fullfile( folder, ...
            sprintf( 'segmentation%i.avi', range ) );
    end
    ...
    elseif( strcmp( mode, 'BoundsFlowInOutOverlay' ) )
        ...
        for( index = 1: length( segmentation ) - 1 )
            ...
            fullframe = [ frame, flowImg; segImg, inoutImg ];
            writeVideo( writerObject, fullframe );
        end
