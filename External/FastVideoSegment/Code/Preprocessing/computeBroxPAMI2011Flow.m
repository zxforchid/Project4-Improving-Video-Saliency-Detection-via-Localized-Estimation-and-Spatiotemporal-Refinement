...
function flow = computeBroxPAMI2011Flow( options, shot )
    ...
    flow = cell( 1, flowframes );     %为每一帧构造一个cell,用来存储光流
    ...
    for( i = startFrame + 1: endFrame )
        ...
        flowframe = int16( mex_LDOF( currImage, nextImage ) );  %<<---------代表什么计算出来的光流场的什么量

        flow{ index }( :, :, 1 ) = flowframe( :, :, 2 );     %<<---------代表什么计算出来的光流场的什么量
        flow{ index }( :, :, 2 ) = flowframe( :, :, 1 );     %<<---------代表什么计算出来的光流场的什么量
