...
function segmentation = videoRapidSegment( options, params, data )
    ...
    %%
    inRatios = getSuperpixelInRatio( superpixels, data.inMaps );%每一个超像素区域占inside-outside map的比例
    ...
    %colours:每一个超像素区域的平均RGB颜色
    %centres：未进行归一化的超像素区域坐标
    [ colours, centres, ~ ] = ...
    ...
    locationUnaries( 1: length( locationMasks ), 1 ) = ...
        locationMasks / ( 0.75 * max( locationMasks ) );
    ...
    for( frame = 1: frames )
        ...
        if( size( uniqueFgColours, 1 ) < fgMix || ...
            ...
            appearanceUnary = getUnaryAppearance( ...
               single( colours( nodeFrameId == frame, : ) ), fgModel, bgModel );

            unaryPotentials( startIndex: stopIndex, : ) = ...
                -log( locationUnaries( startIndex: stopIndex, : ) .* ...
                appearanceUnary );
        end
    end
...
function potentials = computePairwisePotentials( params, superpixels, ...
    flow, colours, centres, labels )
% sSource, sDestination都是在原来的基础上label值减小了1，
% sSoucce中存储是的各个label的值，sDestination是其对应的邻接超像素区域的label
% 使用：假如要求 超像素区域label==1的邻接超像素区域： index=(sSource==l),
%                                                  neighborhood=sDestination(index)
